classdef semantic_grid_map_continuous_S_CSM < handle
    ...
    methods
        function obj = semantic_grid_map_continuous_S_CSM(pose, scan)
            ...
            obj.map.alpha = obj.prior_alpha*ones(obj.map.size,obj.num_classes+1);
        ...
        function build_ogm(obj)
            ...
            for i = 1:obj.map.size
                ...
                [~,maxclass] = max(obj.map.alpha(i,:));
                obj.map.variance(i) = obj.map.alpha(i,maxclass)*(alpha_sum-obj.map.alpha(i,maxclass))/(alpha_sum^2*(alpha_sum+1));
                % -----------------------------------------------
        ...
        function continuous_S_CSM(obj, z, i)
            ...
            elseif z(k,1) < obj.z_max && abs(obj.m_i.range - z(k,1)) < obj.alpha/2 
                ...
                obj.map.alpha(i,z(k,3)) = obj.map.alpha(i,z(k,3))+ker;
            ...
            elseif obj.m_i.range <  z(k,1) && z(k,1) < obj.z_max
                ...
                obj.map.alpha(i,obj.num_classes+1) = obj.map.alpha(i,obj.num_classes+1) + ker_sum;
            end
            % -----------------------------------------------
        end
